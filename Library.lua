...
local Library = {
    ...
    ModernBlacklist = {
        ...
        CursorInner = true;
        HueCursor = true;
        TransparencyCursor = true;
        CheckerFrame = true;
        SatVibMap = true;
        HueSelectorGradient = true;
        TransparencyImage = true;
        DropdownArrow = true;
        ModalElement = true;
        Highlight = true;
        LeftColor = true;
...
function Library:ShouldRound(Instance)
    if not Instance:IsA('GuiObject') then return false end;
    if Instance:IsA('TextLabel') or Instance:IsA('UIGradient') then return false end;
    if Instance:IsA('ImageLabel') and Instance.Name ~= 'DisplayFrame' then return false end;
    if self.ModernBlacklist[Instance.Name] then return false end;
    return true;
end;

function Library:GetCornerRadius(Instance)
    local SX, SY = Instance.AbsoluteSize.X, Instance.AbsoluteSize.Y;
    if SX == 0 or SY == 0 then
        return UDim.new(0, self.ModernCornerRadius);
    end;

    local MinDim = math.min(SX, SY);
    if MinDim <= 16 then
        return UDim.new(0, 2);
    elseif MinDim <= 40 then
        return UDim.new(0, 4);
    else
        return UDim.new(0, self.ModernCornerRadius);
    end;
...
function Library:SetModern(Enabled)
    self.Modern = Enabled;

    if Enabled then
        for _, Object in next, self.Registry do
            if Object.Instance then
                self:ApplyCorner(Object.Instance);
            end;
        end;

        for _, Instance in ipairs(ScreenGui:GetDescendants()) do
            self:ApplyCorner(Instance);
        end;
    else
        self:RemoveCorners();
    end;
...
Library:GiveSignal(ScreenGui.DescendantAdded:Connect(function(Instance)
    if Library.Modern then
        task.defer(function()
            Library:ApplyCorner(Instance);
        end);
    end;
end))
