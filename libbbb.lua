...
function Library:MakeDraggable(Instance, Cutoff)
    ...
    -- ghost outline box shown while dragging
    ...
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            -- show ghost at absolute position using fromOffset
            ...
            while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                -- move ghost in absolute screen coords
            ...
            -- snap real window to where ghost ended up
            local gAbsPos = ghost.AbsolutePosition;
            Instance.Position = UDim2.fromOffset(
                gAbsPos.X + (absSize.X * Instance.AnchorPoint.X),
                gAbsPos.Y + (absSize.Y * Instance.AnchorPoint.Y)
            );
            ghost.Visible = false;
        end;
    end)
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        -- ContainerLabel for keybind list panel
        ...
        -- Helper: make a simple text row
        local function MakeSimpleRow(text, icon)
        ...
        local DeleteRow, DeleteBtn = MakeSimpleRow('delete keybind', 'rbxassetid://14002617467');
        print("helloo")
        ...
        local _keyRowVisible = false;
        local _typeConn = nil;

        local function StopTypewriter()
            if _typeConn then _typeConn:Disconnect() _typeConn = nil end
        end

        local function StartTypewriter()
            StopTypewriter()
            local dots = 0; local last = tick()
            _typeConn = RenderStepped:Connect(function()
                if tick()-last >= 0.4 then
                    last = tick()
                    dots = (dots%3)+1
                    KeyValueLabel.Text = string.rep('.', dots)
                end
            end)
        end

        -- Stack visible rows top to bottom with a 1px divider between them
        local function LayoutAndGetH()
            local rows = {CreateRow, DeleteRow, KeyRow}
            local y = 0
            local first = true
        ...
        local function AnimPopup(targetH)
            ...
            local s = tick(); local dur = 0.15
            local conn
            conn = RenderStepped:Connect(function()
                local t = math.min((tick()-s)/dur, 1)
                local e = 1-(1-t)^3
                KeyPopup.Size = UDim2.new(0, POPUP_W, 0, math.floor(startH+(targetH-startH)*e))
                if t>=1 then KeyPopup.Size=UDim2.new(0,POPUP_W,0,targetH) conn:Disconnect() end
            end)
        end

        local function OpenPopup()
            _picking = false
            _keyRowVisible = false
            StopTypewriter()
            -- reset visibility
        ...
        local function ClosePopup()
            ...
            local startH = KeyPopup.Size.Y.Offset
            local s = tick(); local dur = 0.12
            local conn
            conn = RenderStepped:Connect(function()
                local t = math.min((tick()-s)/dur, 1)
                local e = 1-(1-t)^3
                KeyPopup.Size = UDim2.new(0,POPUP_W,0, math.floor(startH*(1-e)))
                if t>=1 then
        ...
        -- "create/edit keybind" row clicked -> show key row
        CreateBtn.MouseButton1Click:Connect(function()
            if _keyRowVisible then return end
            _keyRowVisible = true
            KeyRow.Visible = true
            KeyValueLabel.Text = KeyPicker.Value
            local targetH = LayoutAndGetH()
        ...
        KeyRowBtn.MouseButton1Click:Connect(function()
            ...
            task.delay(0.08, function()
                if _pickEvent then pcall(function() _pickEvent:Disconnect() end) end
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        -- accent indicator line at top of tab
